...
export async function buildTodaySummary(slot: NotificationSlot): Promise<{ title: string; body: string } | null> {
  ...
  if (slot === 'morning') {
    if (todayEvents.length === 0 && upcomingAssignments.length === 0) return null
    return {
      title: 'Good morning, Guy',
      body: `Today: ${todayEvents.length} event${todayEvents.length !== 1 ? 's' : ''}, ${upcomingAssignments.length} assignment${upcomingAssignments.length !== 1 ? 's' : ''} due this week.`,
    }
  }

  if (slot === 'afternoon') {
    if (urgentItems.length === 0 && unreadCount === 0) return null
    const parts: string[] = []
    ...
    return { title: 'Afternoon check-in', body: parts.join(' · ') }
  }
  ...
  return { title: 'Night check-in', body: bodyParts.join(' ') }
}
